...
static ConfigVar_ptr<int> g_tcp_connect_timeout =
    ...
    _HookIniter() {
        ...
        g_tcp_connect_timeout->addListener([](const int& old_value, const int& new_value) {
            SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) 
                << "tcp connect timeout changed from "
...
static ssize_t do_io(int fd, OriginFun fun, const char* hook_fun_name,
    ...
    FDCtx_ptr ctx = FDManager_single::GetInstance()->get(fd);
    if (!ctx) {
        return fun(fd, std::forward<Args>(args)...);
    }
...
retry:
    ...
    if (n == -1 && errno == EAGAIN) {
        ...
        if (SYLAR_UNLIKELY(rt)) {
            SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << hook_fun_name << " addEvent("
                << fd << ", " << event << ")";
            if (timer) {
                timer->cancel();
            }
...
#undef XX
    ...
    unsigned int sleep(unsigned int seconds) {
        ...
        Fiber_ptr fiber = Fiber::GetThis();
        IOManager* iom = IOManager::GetThis();
        iom->addTimer(seconds * 1000, std::bind((void(Scheduler::*)
                                                 (Fiber_ptr, int thread)) & IOManager::schedule
                                                , iom, fiber, -1));
        Fiber::YieldToHold();
        return 0;
    }
    ...
    int socket(int domain, int type, int protocol) {
        ...
        FDManager_single::GetInstance()->get(fd, true);
        return fd;
    }
    ...
    int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
        ...
        if (rt == 0) {
            ...
            SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << "connect addEvent(" << fd << ", WRITE) error";
        }
    ...
    int close(int fd) {
        ...
        if (ctx) {
            ...
            FDManager_single::GetInstance()->del(fd);
        }
        return close_f(fd);
    }
    ...
    int ioctl(int d, unsigned long int request, ...) {
        ...
        if (FIONBIO == request) {
            ...
            FDCtx_ptr ctx = FDManager_single::GetInstance()->get(d);
            if (!ctx || ctx->isClose() || !ctx->isSocket()) {
    ...
    int setsockopt(int sockfd, int level, int optname, const void* optval, socklen_t optlen) {
        ...
        if (level == SOL_SOCKET) {
            if (optname == SO_RCVTIMEO || optname == SO_SNDTIMEO) {
                FDCtx_ptr ctx = FDManager_single::GetInstance()->get(sockfd);
                if (ctx) {
